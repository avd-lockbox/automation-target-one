...
export const metadata: AutomationMetadata = {
    name: 'download-denials-report',
    version: '0.1.0',
    description: 'Downloads the Denials report from automation-target-one as CSV',
    targetApplication: 'automation-target-one',
    targetApplicationVersion: null,
    category: 'READ',
    parameters: [
        { name: 'dateFrom', type: 'date', required: false, description: 'Filter by denial date (from)' },
        { name: 'dateTo', type: 'date', required: false, description: 'Filter by denial date (to)' },
        { name: 'denialCode', type: 'string', required: false, description: 'Filter by denial code' },
...
export async function execute(context: ExecutionContext): Promise<ExecutionResult> {
    await context.step('Navigate', async (_step) => {
        // TODO (replaces this step.newPage() call): page = await navigateToReport(context.browserContext, context.credentials.url, 'denials');
    ...
    await context.step('Filter', async (_step) => {
        // TODO: await applyDateFilter(page, 'denialDate', {
    ...
    await context.step('Download', async (_step) => {
        ...
        // TODO: await context.artifacts.upload(filePath, { name: 'denials.csv', type: 'FILE' });
